import {
  ...
  OnlineImages,
  ArtworkType,
  SingleLocalProviderType,
  MultiLocalProviderType,
  OnlineProviderType,
} from '../../../models';
import {
  singleLocalProviders,
  multiLocalProviders,
  onlineProviders,
  imageProviders,
...
export function getMaxLength(
  previewAppImage: PreviewDataAppImage,
  onlineImages: OnlineImages[ArtworkType],
) {
  const singleLocalLengths = Object.fromEntries(
    singleLocalProviders.map((providerType: SingleLocalProviderType) => {
      return [
        providerType,
        previewAppImage.singleProviders[providerType] ? 1 : 0,
      ];
    }),
  );
  ...
  const onlineLengths = Object.fromEntries(
  ...
  const singleLength = Object.values(singleLocalLengths).reduce(
    (x, y) => x + y,
    0,
  );
  const multiLength = Object.values(multiLocalLengths).reduce(
    (x, y) => x + y,
    0,
  );
  ...
  const maxLength = Math.max(
    defaultLength,
    singleLength + multiLength + onlineLength,
  );
  return {
    singleLocalLengths,
    multiLocalLengths,
...
export function getImageRanges(
  previewAppImage: PreviewDataAppImage,
  onlineImages: OnlineImages[ArtworkType],
) {
  const lens = getMaxLength(previewAppImage, onlineImages);
  ...
  ranges['default'] = {
    start: 0,
      ...
      lens.singleLength + lens.multiLength + lens.onlineLength == 0
        ? lens.defaultLength
        : 0,
  };
  let integrated = 0;
  for (let n = 0; n < singleLocalProviders.length; n++) {
    ranges[singleLocalProviders[n]] = {
      start: integrated,
      end: integrated + lens.singleLocalLengths[singleLocalProviders[n]],
    };
    integrated = ranges[singleLocalProviders[n]].end;
  }
  for (let n = 0; n < multiLocalProviders.length; n++) {
    ranges[multiLocalProviders[n]] = {
      start: integrated,
      end: integrated + lens.multiLocalLengths[multiLocalProviders[n]],
    };
    integrated = ranges[multiLocalProviders[n]].end;
  }
  for (let n = 0; n < onlineProviders.length; n++) {
    ranges[onlineProviders[n]] = {
      start: integrated,
      end: integrated + lens.onlineLengths[onlineProviders[n]],
    };
