...
export function getGridImagesForTree(
  tree: SteamTree<{ [appId: string]: string }>,
) {
              ...
              fs
                ...
                .then((files) => {
                  ...
                  for (let i = 0; i < files.length; i++) {
                    ...
                    if (
                      data.tree[steamDirectory][userId][appId] === undefined
                    ) {
                      if (extRegex.test(ext))
                        data.tree[steamDirectory][userId][appId] = path.join(
                          gridPath,
                          files[i],
                        );
                    }
