...
export function generateListFromDirectoryList(steamDirectories: string[]) {
  let retrieveMultipleVDFPaths = function (steamDirectories: string[]) {
    let promises: Promise<{
      data: {
        directory: string;
        users: { id: string; paths: { [k: string]: string } }[];
      };
      error: string;
    }>[] = [];
    for (let steamDirectory of steamDirectories) {
      promises.push(
        new Promise<{
          ...
          getAvailableLogins(steamDirectory)
            .then((userAccounts: userAccountData[]) => {
              if (userAccounts.length === 0) {
                resolve({
                  data: null,
                  error: APP.lang.helpers.error.noUserIdsInDir__i.interpolate({
                ...
                resolve({
                  data: { directory: steamDirectory, users },
                  error: null,
                });
              }
            })
  ...
  return retrieveMultipleVDFPaths(steamDirectories).then((data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      if (data[i].error) errors.push(data[i].error);
      else {
