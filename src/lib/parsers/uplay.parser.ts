      ...
      installDirDictPromise
        .then((installDirDict: { [key: string]: any }) => {
          ...
          this.generateHexArray(configHex).forEach((hexStr: string) => {
            const line = Buffer.from(hexStr, 'hex')
              .toString('utf8')
              .replace(/\n/g, '');
            ...
            if (!end) {
              ...
              if (
                line.substr(0, 2) === '  ' &&
                !line.includes('sort_string:')
              ) {
                game.push(line);
              }
            }
          });
          let parsedGames = finalOutput
            .filter(
              (x) =>
                x &&
                x.root &&
                x.root.start_game &&
                !x.root.third_party_platform,
            )
            .map((x) => x.root);
          parsedGames.forEach((item: any) => {
            let basePath = (item.start_game.offline || item.start_game.online)
              .executables[0].path.relative;
            if (
              (item.installer || {}).game_identifier &&
              item.launcher_id &&
