...
export class GlobParser implements GenericParser {
  ...
  private validate(fileGlob: string) {
    ...
    if (match === null) return this.lang.errors.noTitle__md;
    else if (match.length > 3) return this.lang.errors.moreThanOneTitle__md;

    testRegExpr = /.*\*\${title}.*|.*\${title}\*.*/i;
    match = testRegExpr.exec(fileGlob);
    if (match !== null) return this.lang.errors.noStarNextToTitle__md;

    testRegExpr = /.*\?\${title}.*|.*\${title}\?.*/i;
    match = testRegExpr.exec(fileGlob);
    if (match !== null) return this.lang.errors.noAnyCharNextToTitle__md;
  ...
  private extractTitle(titleData: TitleTagData, file: string) {
    ...
    if (file !== undefined) {
      ...
      if (titleMatch !== null && titleMatch[titleData.titleRegex.pos])
        return titleMatch[titleData.titleRegex.pos]
          .replace(/\//g, path.sep)
          .trim();
    }
    return undefined;
