        ...
        itchIoWindowsOnLinuxInstallDriveRedirect: {
          ...
          validationFn: (input: string) => {
            if (
              ...
              return this.lang.errors
                .invalidItchIoWindowsOnLinuxInstallDriveRedirect;
            }
          },
          info: this.lang.docs__md.input.join(''),
        },
  ...
  execute(
    ...
    return new Promise<ParsedData>((resolve, reject) => {
      try {
        ...
        const itchIoAppDataDir =
          inputs.itchIoAppDataOverride ||
          (() => {
            switch (os.type()) {
              case 'Windows_NT':
            ...
            const success = games
              .map(({ title, verdict }: { [key: string]: string }) => {
                ...
                if (os.type() == 'Windows_NT') {
                  ...
                  os.type() == 'Linux' &&
                  candidates[0].flavor == 'windows' &&
                  inputs.itchIoWindowsOnLinuxInstallDriveRedirect
                ) {
