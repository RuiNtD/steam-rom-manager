    ...
    return new Promise<ParsedData>(async (resolve, reject) => {
      ...
      try {
        ...
        for (let userdir of directories) {
          ...
          const addedItemsPath = path.join(
            userdir,
            'config',
            'addedItemsV2.json',
          );
          ...
          const addedAppIds = Object.keys(addedApps).filter(
            (appId) => !addedApps[appId].artworkOnly,
          );
          if (fs.existsSync(shortcutsPath)) {
            const { shortcuts } = shortcutsParser.parseBuffer(
              fs.readFileSync(shortcutsPath),
            );
            const mappedApps = shortcuts
              .filter((shortcut: any) => {
                return !addedAppIds.includes(
                  steam.appifyShortcutId(shortcut.appid),
                );
              })
              .map((shortcut: any) => {
                return {
                  extractedTitle: json.caselessGet(shortcut, [['AppName']]),
                  extractedAppId: steam.shortenAppId(
                    steam.appifyShortcutId(shortcut.appid),
                  ),
                };
              });
