...
export class GlobRegexParser implements GenericParser {
  ...
  private validate(fileGlob: string) {
    ...
    if (match === null) return this.lang.errors.noRegex__md;

    testRegExpr = /\${.*?}/i;
    match = testRegExpr.exec(fileGlob);
    if (match.length > 1) return this.lang.errors.moreThanOneRegex__md;

    testRegExpr = /.*\*\${.*?}.*|.*\${.*?}\*.*/i;
    match = testRegExpr.exec(fileGlob);
    if (match !== null) return this.lang.errors.noStarNextToRegex__md;

    testRegExpr = /.*\?\${.*?}.*|.*\${.*?}\?.*/i;
    match = testRegExpr.exec(fileGlob);
    if (match !== null) return this.lang.errors.noAnyCharNextToRegex__md;

    let fileGlobWithoutRegex = fileGlob.replace(/\${.*?}/i, '');

    testRegExpr = /.*\*\*.+\${.*?}.+\*\*.*/i;
    match = testRegExpr.exec(fileGlob);
    if (match !== null) return this.lang.errors.noGlobstarOnBothSides__md;

    testRegExpr = /.*\{.*?\/+.*?\}.*\${.*?}.*\{.*?\/+.*?\}.*/i;
    match = testRegExpr.exec(fileGlob);
    if (match !== null) return this.lang.errors.noBracedDirSetOnBothSides__md;
    ...
    while ((match = testRegExpr.exec(fileGlobWithoutRegex)) !== null) {
      if (match[2].length === 0) return this.lang.errors.noEmptyPattern__md;
    }

    testRegExpr = /\[(.*?)\]/g;
    while ((match = testRegExpr.exec(fileGlobWithoutRegex)) !== null) {
      if (match[1].length === 0) return this.lang.errors.noEmptyCharRange__md;
    }
  ...
  private getTitleDepth(fileGlob: string) {
    let depth: { direction: 'left' | 'right'; level: number } = {
      direction: undefined,
      level: undefined,
    };
  ...
  private getTitleRegex(fileGlob: string) {
    ...
    let getRegexString = (segment: string) => {
      ...
      if (mm.empty) return '^\\s*?$';
      else {
        return (mm.makeRe() || {}).source;
      }
    };

    let titleSegmentMatch = fileGlob.match(
      /.*\/(.*\${.*?}.*?)\/|.*\/(.*\${.*?}.*)|(.*\${.*?}.*?)\/|(.*\${.*?}.*)/i,
    );
  ...
  private makeRegexRegex(fileGlob: string) {
    ...
    } else return { regex: new RegExp(''), replaceText: '' };
  }

  private extractTitleTag(fileGlob: string) {
    let extractedData: TitleTagData = {
      finalGlob: undefined,
      globRegex: undefined,
      titleRegex: undefined,
      depth: undefined,
    };
    extractedData.depth = this.getTitleDepth(fileGlob);
    extractedData.titleRegex = this.getTitleRegex(fileGlob);
    extractedData.finalGlob = this.getFinalGlob(
      fileGlob,
      extractedData.depth.level,
    );
  ...
  private extractTitle(titleData: TitleTagData, file: string) {
    if (titleData.depth.level !== null) {
      ...
      file =
        fileSections[
          titleData.depth.direction === 'right'
            ? fileSections.length - (titleData.depth.level + 1)
            : titleData.depth.level
        ];
    }
    ...
    if (file !== undefined) {
      ...
      if (titleMatch !== null && titleMatch[titleData.titleRegex.pos]) {
        if (typeof titleData.globRegex.replaceText === 'string') {
          let title = titleMatch[titleData.titleRegex.pos]
            .replace(titleData.globRegex.regex, titleData.globRegex.replaceText)
            .replace(/\//g, path.sep)
            .trim();
          return title.length > 0 ? title : undefined;
        } else {
          titleMatch = titleMatch[titleData.titleRegex.pos].match(
            titleData.globRegex.regex,
          );
          if (titleMatch !== null) {
            let title: string = '';
            for (let i = 1; i < titleMatch.length; i++) {
              if (titleMatch[i]) title += titleMatch[i];
            }
            if (title.length === 0)
              return titleMatch[0].replace(/\//g, path.sep).trim();
            else return title.replace(/\//g, path.sep).trim();
          }
        }
      }
    }
  ...
  private extractTitles(
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      else parsedData.failed.push(filePath);
    }
    return parsedData;
  }
  ...
  execute(
    ...
    return new Promise<ParsedData>((resolve, reject) => {
      ...
      if (validationText === null) {
        ...
        glob(titleData.finalGlob, { dot: true, cwd: directory, follow: true })
          .then((files: string[]) => {
            ...
            resolve(
              this.extractTitles(
                titleData,
                directory as string,
                drive.test(directory)
                  ? files.map((x) => x.replace(driveReplace, ''))
                  : files,
              ),
            );
          })
