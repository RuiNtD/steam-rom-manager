...
export class VDF_Manager {
  ...
  prepare(data: SteamDirectory[] | PreviewData) {
    return new Promise<void>((resolve, reject) => {
      ...
      if (data instanceof Array) {
        if (data.length > 0) {
          chain = chain.then((data: SteamDirectory[] | PreviewData) =>
            vdf.generateListFromDirectoryList(data as SteamDirectory[]),
          );
        } else {
      ...
      return chain
        ...
        .catch((error) => {
          reject(
            new VDF_Error(
              this.lang.error.couldNotPrepareToRead__i.interpolate({ error }),
            ),
          );
        });
    });
  ...
  getBatchProgress() {
    ...
    for (let steamDirectory in this.data) {
      for (let userId in this.data[steamDirectory]) {
        updates.push(
          this.data[steamDirectory][userId].screenshots.getBatchProgress().pipe(
            map((b: { batch: number; total: number }) => {
              return {
                update: `Doing batch (${b.batch + 1}/${b.total}) for user ${userId}`,
                batch: b.batch,
              };
            }),
          ),
  ...
  backup(options?: { shortcuts?: boolean; screenshots?: boolean }) {
    return new Promise<void>((resolve, reject) => {
      ...
      let backupScreenshots =
        options !== undefined ? options.screenshots : true;

      for (let steamDirectory in this.data) {
        for (let userId in this.data[steamDirectory]) {
          if (backupShortcuts) {
            promises.push(
              this.data[steamDirectory][userId].shortcuts.backup(
                'backup',
                true,
              ),
            );
            promises.push(
              this.data[steamDirectory][userId].shortcuts.backup('firstbackup'),
            );
          }
          if (backupScreenshots) {
            promises.push(
              this.data[steamDirectory][userId].screenshots.backup(
                'backup',
                true,
              ),
            );
      ...
      return Promise.all(promises)
        ...
        .catch((error) =>
          reject(
            this.lang.error.couldNotBackupEntries__i.interpolate({ error }),
          ),
        );
    });
  }
  ...
  read(options?: {
    shortcuts?: { skipIndexing: boolean; read: boolean };
    addedItems?: boolean;
    screenshots?: boolean;
  }) {
    return new Promise<void>((resolve, reject) => {
      ...
      for (let steamDirectory in this.data) {
        for (let userId in this.data[steamDirectory]) {
          if (readShortcuts)
            promises.push(
              this.data[steamDirectory][userId].shortcuts.read(skipIndexing),
            );
          if (readAddedItems)
    ...
    return new Promise<{
        ...
        for (let userId in this.data[steamDirectory]) {
          if (writeShortcuts) {
            shortcutPromises.push(
              this.data[steamDirectory][userId].shortcuts.write(),
            );
          }
          if (writeAddedItems) {
            addedItemsPromises.push(
              this.data[steamDirectory][userId].addedItems.write(),
            );
          }
          if (writeScreenshots) {
            screenshotPromises.push(
              this.data[steamDirectory][userId].screenshots
                .write(batch, batchSize, dnsServers)
                .then((outcome: VDF_ScreenshotsOutcome) => {
      ...
      this.artworkCache
        ...
        .then((errors: VDF_Error[]) => {
          ...
          resolve({
            nonFatal: realErrors.length
              ? new VDF_Error(realErrors, this.lang.error.nonFatal, true)
              : undefined,
      ...
      Promise.resolve()
        .then(() => {
          ...
          this.forEach((steamDirectory, userId, listItem) => {
            if (
              listItem.shortcuts.invalid ||
              listItem.addedItems.invalid ||
              listItem.screenshots.invalid
            )
              return;
            let apps = previewData[steamDirectory][userId].apps;
            let currentAppIds = Object.entries(
              previewData[steamDirectory][userId].apps,
            ).map(([appId, app]: [appId: string, app: PreviewDataApp]) => {
              if (app.changedId) {
            ...
            let enabledParsers = Array.from(
              new Set(
                currentAppIds.map((appid: string) => {
                  if (apps[appid]) {
                    return apps[appid].parserId;
                  } else {
                    return Object.values(apps).filter(
                      (app: PreviewDataApp) => app.changedId == appid,
                    )[0].parserId;
                  }
                }),
              ),
            ...
            if (!deleteDisabledShortcuts) {
              addedAppIds = addedAppIds.filter((appid: string) =>
                enabledParsers.includes(addedApps[appid].parserId),
              );
            }
            if (!addedCategories[steamDirectory]) {
              addedCategories[steamDirectory] = {};
            }
            addedCategories[steamDirectory][userId] = Object.fromEntries(
              addedAppIds.map((appId) => [
                steam.shortenAppId(appId),
                addedApps[appId].categories,
              ]),
            );
            ...
            extraneousAppIds[steamDirectory][userId] = addedAppIds.filter(
              (appid: string) => !currentAppIds.includes(appid),
            );
            listItem.screenshots.extraneous =
              extraneousAppIds[steamDirectory][userId];
            listItem.shortcuts.extraneous =
              extraneousAppIds[steamDirectory][userId];
            ...
            for (let appId in apps) {
              ...
              if (app.status === 'add') {
                ...
                const artworkOnly = superTypes[ArtworkOnlyType].includes(
                  app.parserType,
                );
                listItem.addedItems.addItem(
                  appId,
                  app.parserId,
                  artworkOnly,
                  app.steamCategories,
                ...
                for (const artworkType of artworkTypes) {
                  const currentImage = appImage.getCurrentImage(
                    ...
                    onlineImages[artworkType],
                  );
                  if (
                    currentImage !== undefined &&
                    currentImage.imageProvider !== imageProviderNames.steam
                  ) {
                    listItem.screenshots.addItem({
                      appId: steam
                        .shortenAppId(appId)
                        .concat(artworkIdDict[artworkType]),
                      title: app.title,
                  ...
                  if (
                    currentImage !== undefined &&
                    currentImage.imageProvider == imageProviderNames.sgdb
                  ) {
                    this.artworkCache.cacheArtwork(
                      currentImage.imageGameId,
                      currentImage.imageArtworkId,
                      appId,
                      artworkType,
                    );
                  }
                  ...
                  if (artworkType === 'icon') {
                    ...
                    if (currentImage !== undefined) {
                      let icon_ext: string = currentImage.imageUrl
                        .split('.')
                        .slice(-1)[0];
                      icon_ext = steam.map_ext['' + icon_ext] || icon_ext;
                      icon_path = path.join(
                        listItem.screenshots.gridDir,
                        `${steam.shortenAppId(appId).concat('_icon')}.${icon_ext}`,
                      );
                    }
                    if (!artworkOnly && item !== undefined) {
                      (item.appid = steam.generateShortcutId(
                ...
                for (const artworkType of artworkTypes) {
                  listItem.screenshots.removeItem(
                    steam
                      .shortenAppId(appId)
                      .concat(artworkIdDict[artworkType]),
                  );
          ...
          resolve({
            extraneousAppIds: extraneousAppIds,
        ...
        .catch((error: Error) => {
          reject(
            new VDF_Error(
              this.lang.error.couldNotMergeEntries__i.interpolate({ error }),
            ),
          );
        });
    });
      ...
      Promise.resolve()
        .then(() => {
          ...
          this.forEach((steamDirectory, userId, listItem) => {
            ...
            extraneousAppIds[steamDirectory][userId] = Object.keys(
              addedApps,
            ).filter((appId) => !addedApps[appId].artworkOnly);
            if (
              listItem.shortcuts.invalid ||
              listItem.addedItems.invalid ||
              listItem.screenshots.invalid
            ...
            for (const appId in addedApps) {
              ...
              for (let artworkType of artworkTypes) {
                listItem.screenshots.removeItem(
                  steam.shortenAppId(appId).concat(artworkIdDict[artworkType]),
                );
              }
            }
        ...
        .catch((error: Error) => {
          reject(
            new VDF_Error(
              this.lang.error.couldNotRemoveEntries__i.interpolate({ error }),
            ),
          );
        });
    });
