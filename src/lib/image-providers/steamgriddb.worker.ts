      ...
      idPromise
        .then((chosenId: number | undefined) => {
          if (!chosenId) {
            ...
            const choose = (x: any, fallback: string[]) =>
              x && x.length ? x : fallback;
            if (self.proxy.imageType === 'long') {
              query = self.client.getGrids(
                Object.assign(params, {
                  dimensions: choose(self.proxy.imageProviderAPIs.sizes, [
                    '460x215',
                    '920x430',
                  ]),
              ...
              query = self.client.getHeroes(
                Object.assign(params, {
                  dimensions: choose(
                    self.proxy.imageProviderAPIs.sizesHero,
                    null,
                  ),
              ...
              query = self.client.getLogos(
                Object.assign(params, {
                  styles: self.proxy.imageProviderAPIs.stylesLogo,
                }),
              );
            } else if (self.proxy.imageType === 'icon') {
              query = self.client.getIcons(
                Object.assign(params, {
                  dimensions: choose(
                    self.proxy.imageProviderAPIs.sizesIcon,
                    null,
                  ),
