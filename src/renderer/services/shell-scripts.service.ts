...
export class ShellScriptsService {
  ...
  private downloadFinished: BehaviorSubject<boolean> = new BehaviorSubject(
    false,
  );
  ...
  async download(force: boolean = false) {
    if (!this.downloadStatus.getValue()) {
      ...
      let scriptURLs = treeData.tree
        .filter(
          (entry: any) => path.dirname(entry.path) == 'files/shellscripts',
        )
      ...
      for (let scriptURL of scriptURLs) {
        const cleanURL = scriptURL
          .split('/')
          .map((x: string) => encodeURI(x))
          .join('/');
        const fullURL = this.rawURL.concat(cleanURL);
        const script = await this.xRequest.request(fullURL, {
          ...this.requestOpts,
          responseType: 'text',
        });
