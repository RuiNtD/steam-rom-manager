...
export class CustomVariablesService {
  ...
  private rawURL: string =
    'https://raw.githubusercontent.com/SteamGridDB/steam-rom-manager/master/';
  ...
  async download(force: boolean = false): Promise<void> {
    if (!this.downloadStatus.getValue()) {
      ...
      let variableURLs = treeData.tree
        .filter(
          (entry: any) => path.dirname(entry.path) == 'files/customvariables',
        )
        .map((entry: any) => entry.path);
      let customVariables: any[] = [];
      for (let variableURL of variableURLs) {
        const cleanURL = variableURL
          .split('/')
          .map((x: string) => encodeURI(x))
          .join('/');
        const fullURL = this.rawURL.concat(cleanURL);
        const customVariable = await this.xRequest.request(
          fullURL,
          this.requestOpts,
        );
      ...
      if (error) {
        this.loggerService.error(
          this.lang.error.failedToDownload__i.interpolate({ error: error }),
        );
      } else {
  ...
  async load() {
    try {
      await this.download();
      const srmVariables = await json.read<CustomVariables>(
        paths.customVariables,
      );
      if (!fs.existsSync(paths.userVariables)) {
        await json.write(paths.userVariables, {});
      }
      const userVariables = await json.read<CustomVariables>(
        paths.userVariables,
      );
      const data = Object.fromEntries(
        _.uniq([
          ...Object.keys(srmVariables),
  ...
  save(force: boolean = false) {
    if (!this.savingIsDisabled || force) {
      json
        .write(paths.customVariables, this.variableData.getValue())
        .then()
        .catch((error) => {
