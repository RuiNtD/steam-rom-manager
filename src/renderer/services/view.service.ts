...
import { VDF_Manager, CategoryManager, ControllerManager } from '../../lib';
  ...
  async refreshGames(force?: boolean) {
    ...
    for (const steamDirectory in this.vdfData) {
      ...
      for (const userId in this.vdfData[steamDirectory]) {
        try {
          this.loggerService.info(
            `Reading category information for user ${userId} (View Games).`,
          );
          this.categoryData[steamDirectory][userId] =
            await categoryManager.readCategories(steamDirectory, userId);
        } catch (e) {}
        const configsetDir = ControllerManager.configsetDir(
          steamDirectory,
          userId,
        );
      ...
      for (const controllerType of controllerTypes) {
        this.controllerTemplateData[steamDirectory][controllerType] =
          await ControllerManager.readTemplates(steamDirectory, controllerType);
      }
    }
