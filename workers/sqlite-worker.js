process.on('message', (data) => {
  try {
    ...
    if (task === 'gog-galaxy') {
      ...
      const playtaskparams = db
        .prepare('select * from PlayTaskLaunchParameters')
        .all();
      ...
      playtasks = playtasks
        ...
        .filter(
          (x) =>
            x.productType == 'gog' ||
            (options.externals && x.productType == 'generic'),
        )
      ...
      const query =
        'select ProductTitle, InstallDirectory, Installed, Id from DbSet';
      process.send({
        type: 'result',
